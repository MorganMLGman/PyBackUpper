...
class BackupManager(metaclass=Singleton):
    ...
    def get_src_size(self) -> int:
        ...
        """
        logger.debug(f"Getting size of {self.src_path}...")
        size = 0
        ...
        logger.debug(f"Size of {self.src_path} is {size_to_human_readable(size)}.")
        return size
    ...
    def get_dest_space(self):
        ...
        """
        logger.debug(f"Getting available space of {self.dest_path}...")
        ...
        try:
            ...
            if total == 0:
                logger.error(f"Error getting available space of {self.dest_path}.")
                return 0

            if space < total * 0.1:
                logger.warning(f"Available space of {self.dest_path} is less than 10% of total space.")
        ...
        logger.debug(f"Available space of {self.dest_path} is {size_to_human_readable(space)}.")
        return space
    ...
    def save_backup_info(self, dest_path:str=None) -> None:
        ...
        logger.debug(f"Saving backup info to {dest_path}...")
        dest_path = normpath(dest_path)

        if not exists(dest_path):
            logger.warning(f"Path {dest_path} does not exist. Creating it.")
            try:
                makedirs(dest_path)
            except OSError as e:
                logger.error(f"Path {dest_path} cannot be created. {e}")
    ...
    def load_backup_info(self, src_path:str=None, ignore_hash_mismatch:bool=True) -> None:
        ...
        logger.debug(f"Loading backup info from {path}...")
            ...
            if tmp_backup.completed and tmp_backup.compressed:
                if (backup["raw_hash"] != tmp_backup.calculate_raw_hash(method="sha256")) or \
                    ...
                    if ignore_hash_mismatch:
                        logger.warning(f"Hash mismatch for backup {backup['name']}. Loading backup anyway.")
                    else:
                        logger.error(f"Hash mismatch for backup {backup['name']}. Skipping backup.")
                        continue
        ...
        logger.debug(f"Backup info loaded from {src_path}.")     
    ...
    def restore_backup_info(self, src_path:str=None) -> None:
        ...
        logger.debug(f"Restoring backup info from {src_path}...")
        src_path = normpath(src_path)
    ...
    def create_backup(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Backup {name} already exists.")
            return False
        except FileNotFoundError:
        ...
        try:
            ...
            logger.error(f"Error calculating raw hash of {backup.name}.")
            return False

        logger.debug(f"Raw hash of {backup.name} is {raw_hash}.")
        ...
        if self.compressed_to_keep > 0:
            try:
                ...
                logger.error(f"Error compressing backup {backup.name}.")
                return False

            logger.debug(f"Compressed hash of {backup.name} is {compressed_hash}.")
            logger.debug(f"Compression ratio of {backup.name} is {backup.calculate_compression_ratio():.2f}.")

        self.backups["local"].append(backup)

        logger.info(f"Backup {backup.name} created.")
        logger.debug(f"Backup {backup.name} size is {size_to_human_readable(backup.get_size())}.")
        return True
    ...
    def upload_backup_to_s3(self, backup_name:str) -> bool:
        ...
        if self.s3_handler is None:
            logger.error("S3 handler is not set.")
            return False

        logger.debug(f"Uploading backup {backup_name} to S3...")
    ...
    def download_backup_from_s3(self, backup_name:str) -> bool:
        ...
        if self.pending_backup:
            logger.error("Backup task is running, need to wait for it to finish.")
            return False
        ...
        logger.debug(f"Downloading backup {backup_name} from S3...")

        for backup in self.backups["local"]:
            if backup.name == backup_name and backup.compressed:
                logger.debug(f"Backup {backup_name} already exists.")
                self.pending_backup = False
                return True
        ...
        try:
            ...
            logger.exception(f"Error downloading backup {backup_name} from S3. {e}", exc_info=True)
            self.pending_backup = False
            return False

        if not exists(self.dest_path + "/" + backup_name + ".zip"):
            logger.error(f"Downloaded backup {backup_name} not found.")
            self.pending_backup = False
            return False
        ...
        try:
            ...
            logger.exception(f"Error saving backup info. {e}", exc_info=True)
        self.pending_backup = False
        return True
    ...
    def delete_raw_backup(self, backup_name:str) -> bool:
        ...
        """
        logger.debug(f"Deleting raw backup {backup_name}...")
        index = self.get_backup_index_by_name(backup_name)
        ...
        try:
            ...
            logger.exception(f"Error updating backup {backup_name} in S3. {e}", exc_info=True)
            pass

        logger.debug(f"Raw backup {backup_name} deleted.")
        return True
    ...
    def delete_compressed_backup(self, backup_name:str) -> bool:
        ...
        Returns:
            bool: True if the backup has been deleted, False otherwise.
        """
        logger.debug(f"Deleting compressed backup {backup_name}...")
        index = self.get_backup_index_by_name(backup_name)
        ...
        logger.debug(f"Compressed backup {backup_name} deleted.")
        return True
    ...
    def delete_s3_backup(self, backup_name:str) -> bool:
        ...
        logger.debug(f"Deleting S3 backup {backup_name}...")
        ...
        try:
            ...
            logger.exception(f"Error deleting backup {backup_name} from S3. {e}", exc_info=True)
            return False

        logger.debug(f"S3 backup {backup_name} deleted.")
        return True
    ...
    def delete_backup(self, backup_name:str) -> bool:
        ...
        if self.pending_backup:
            ...
            return False
        self.pending_backup = True
        logger.debug(f"Deleting backup {backup_name}...")
        index = self.get_backup_index_by_name(backup_name)
        ...
        logger.debug(f"Backup {backup_name} deleted.")
        try:
            self.save_backup_info()
        except Exception as e:
            logger.exception(f"Error saving backup info. {e}", exc_info=True)
    ...
    def delete_old_backups(self) -> None:
        ...
        logger.debug(f"Old backups deleted.")

    def clear_dest_path(self) -> None:
        logger.debug(f"Clearing {self.dest_path}...")
        for item in listdir(self.dest_path):
            ...
            if isfile(item_path):
                ...
                rmtree(item_path)
        logger.debug(f"{self.dest_path} cleared.")
    ...
    def unzip_backup(self, backup_name:str) -> bool:
        ...
        if self.pending_backup:
            ...
            return False
        self.pending_backup = True
        logger.debug(f"Unzipping backup {backup_name}...")
        index = self.get_backup_index_by_name(backup_name)
        ...
        if not exists(join(self.dest_path, backup_name)):
            logger.error(f"Unzipped backup {backup_name} not found.")
            return False

        logger.debug(f"Backup {backup_name} unzipped.")
    ...
    def restore_backup(self, backup_name:str, restore_path:str) -> bool:
        ...
        if self.pending_backup:
            ...
            return False
        self.pending_backup = True
        logger.debug(f"Restoring backup {backup_name}...")
        index = self.get_backup_index_by_name(backup_name)
        ...
        try:
            ...
            logger.exception(f"Error restoring backup {backup_name}. {e}", exc_info=True)
            return False
        except shutilError as e:
            logger.exception(f"Error restoring backup {backup_name}. {e}", exc_info=True)
            return False
        ...
        if dircmp(self.src_path, join(self.dest_path, backup_name)).diff_files != []:
            logger.error(f"Error restoring backup {backup_name}.")
            return False

        logger.debug(f"Backup {backup_name} restored.")
    ...
    def run_backup(self, callback=None) -> str:
        ...
        """
        if self.pending_backup:
            logger.error("A backup is already running.")
            return None
        self.pending_backup = True
